...
class WC_Gateway_IQ_Instapago_Commerce extends WC_Payment_Gateway
{
	...
	{
		$this->form_fields = include 'includes/settings-iq-instapago.php';
	}
	...
	private function checkResponseCode($response)
	{
		...
		switch ($code) {
			case 400:
				wc_add_notice(__('Error al validar los datos enviados.', 'iq_instapago'), 'error');
				break;
			case 401:
				wc_add_notice(__('Error de autenticación, ha ocurrido un error con las llaves utilizadas.', 'iq_instapago'), 'error');
				break;
			case 403:
				wc_add_notice(__('Pago Rechazado por el banco.', 'iq_instapago'), 'error');
				break;
			case 500:
				wc_add_notice(__('Ha Ocurrido un error interno dentro del servidor.', 'iq_instapago'), 'error');
				break;
			case 503:
				wc_add_notice(__('Ha Ocurrido un error al procesar los parámetros de entrada. Revise los datos enviados y vuelva a intentarlo.', 'iq_instapago'), 'error');
				break;
			case 201:
				return [
					'code'      => $code,
