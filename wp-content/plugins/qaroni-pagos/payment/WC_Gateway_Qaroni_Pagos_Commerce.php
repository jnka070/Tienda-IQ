...
class WC_Gateway_Qaroni_Pagos_Commerce extends WC_Payment_Gateway
{
	...
	{
		$this->form_fields = include 'includes/settings-qaroni-pagos.php';
	}
	...
	{
		$icon_html = '<img src="' . plugins_url('qaroni-pagos/public/img/qaroni-logo-black.webp') . '" alt="qaroni-pagos" style="width: auto; height: 40px;">';
	...
	public function payment_fields()
	{
		...
		echo '<p class="qaroni_pagos-form--txt-help">'. $this->description.'</p>';
	...
	public function process_payment($order_id)
	{
		...
		if ($result['code'] == 201) {
			...
			update_post_meta($order_id, 'qaroni_pagos_voucher', $result['voucher']);
			update_post_meta($order_id, 'qaroni_pagos_bank_ref', $result['reference']);
			update_post_meta($order_id, 'qaroni_pagos_id_payment', $result['id_pago']);
			update_post_meta($order_id, 'qaroni_pagos_bank_msg', $result['msg_banco']);
			update_post_meta($order_id, 'qaroni_pagos_sequence', $result['sequence']);
			update_post_meta($order_id, 'qaroni_pagos_approval', $result['approval']);
			update_post_meta($order_id, 'qaroni_pagos_lote', $result['lote']);
	...
	private function checkResponseCode($response)
	{
		...
		switch ($code) {
			case 400:
				wc_add_notice(__('Error al validar los datos enviados.', 'qaroni_pagos'), 'error');
				break;
			case 401:
				wc_add_notice(__('Error de autenticación, ha ocurrido un error con las llaves utilizadas.', 'qaroni_pagos'), 'error');
				break;
			case 403:
				wc_add_notice(__('Pago Rechazado por el banco.', 'qaroni_pagos'), 'error');
				break;
			case 500:
				wc_add_notice(__('Ha Ocurrido un error interno dentro del servidor.', 'qaroni_pagos'), 'error');
				break;
			case 503:
				wc_add_notice(__('Ha Ocurrido un error al procesar los parámetros de entrada. Revise los datos enviados y vuelva a intentarlo.', 'qaroni_pagos'), 'error');
				break;
			case 201:
				return [
					'code'      => $code,
				...
				break;
			default:
				wc_add_notice(__('Error general.','qaroni_pagos'), 'error');
				break;
		}
	}
}
